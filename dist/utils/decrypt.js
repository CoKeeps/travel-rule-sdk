...
export async function decryptJwePayload(vasp_key, payloadJwe) {
    ...
    const isValidType = typeof payloadJwe === 'string' ||
        payloadJwe instanceof Uint8Array ||
        (payloadJwe !== null && typeof payloadJwe === 'object');
    if (!isValidType) {
        throw new ValidationError(`payloadJwe must be a string, Uint8Array, or object. Received: ${typeof payloadJwe}`, 'payloadJwe');
    }
    ...
    try {
        ...
        try {
            ...
            throw new SDKError(`Failed to parse decrypted payload as JSON: ${parseError.message}`, parseError instanceof Error ? parseError : undefined);
        }
    }
    catch (error) {
        ...
        if (error instanceof Error) {
            if (error.message.includes('decryption') || error.message.includes('decrypt')) {
                throw new SDKError(`Decryption failed: ${error.message}. Check that the encryption key matches the payload.`, error);
            }
            if (error.message.includes('import') || error.message.includes('key')) {
                throw new SDKError(`Failed to import encryption key: ${error.message}. Check that enc.priv is valid.`, error);
            }
            throw new SDKError(`Failed to decrypt JWE payload: ${error.message}`, error);
        }
