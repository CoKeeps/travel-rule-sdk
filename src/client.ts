...
import {
  validateVaspKeysWithError,
  validateMessageFormData,
  MessageFormData,
  VaspKeysWithMaterial,
} from './utils/validate';
...
export class TravelSDKClient {
  ...
  private ensureVaspKeysSet(): void {
    if (!this.vaspKeys) {
      throw new ValidationError(
        'vaspKeys must be set using setVaspKeys() before calling this method',
        'vaspKeys',
      );
    }
  }
  ...
  private parseApiError(error: unknown): SDKError | APIError {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (!response) {
        return new SDKError(`Network error: ${axiosError.message}`, axiosError);
      }
      ...
      if (data) {
        if (typeof data === 'string') {
          ...
          const desc = maybeObj.error_description;
      ...
      const message =
        errorMessage !== axiosError.message ? errorMessage : `HTTP ${status} ${statusText}`;

      return new SDKError(message, axiosError);
    }
  ...
  private getTargetUrl(
    purpose: 'token' | 'get-message' | 'send-message',
    messageID?: string,
  ): string {
    switch (purpose) {
      case 'token':
      ...
      default:
        throw new ValidationError(
          `Invalid purpose: ${purpose}. Must be 'token', 'get-message', or 'send-message'`,
          'purpose',
        );
    }
  }
  ...
  setVaspKeys(vaspKeys: VaspKeys): void {
    ...
    const validation = validateVaspKeysWithError(vaspKeys as Partial<VaspKeysWithMaterial>);
