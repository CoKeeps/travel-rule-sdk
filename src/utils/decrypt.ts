...
export async function decryptJwePayload(vasp_key: VaspKeys, payloadJwe: string | Uint8Array) {
  if (!vasp_key?.enc?.priv) {
    throw new ValidationError(
      'Encryption private key (enc.priv) is required in vaspKeys',
      'vaspKeys.enc.priv',
    );
  }
  if (!payloadJwe) {
  ...
  try {
    ...
    try {
      ...
      throw new SDKError('Failed to parse decrypted payload as JSON.', undefined);
    }
  } catch (error: unknown) {
