...
interface JWKWithOth extends JWK {
  oth?: unknown;
}

export function stripPrivateJwkParams(jwk: JWK): JWK {
  const cleaned: JWKWithOth = { ...jwk };
  delete cleaned.d;
  delete cleaned.p;
  delete cleaned.q;
  delete cleaned.dp;
...
export async function genClientAssertion(
  ...
  if (!vasp_key?.sig?.priv) {
    throw new ValidationError(
      'Signature private key (sig.priv) is required in vaspKeys',
      'vaspKeys.sig.priv',
    );
  }
...
export async function genDpopProof(
  ...
  if (!vasp_key?.dpop?.priv) {
    throw new ValidationError(
      'DPoP private key (dpop.priv) is required in vaspKeys',
      'vaspKeys.dpop.priv',
    );
  }
  if (!tokenEndpoint || typeof tokenEndpoint !== 'string' || !tokenEndpoint.trim()) {
  ...
  try {
    ...
    if (accessToken) {
      if (typeof accessToken !== 'string' || !accessToken.trim()) {
        throw new ValidationError(
          'accessToken must be a non-empty string if provided',
          'accessToken',
        );
      }
      ...
      const ath = hashBase64.replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_');
  ...
  if (type === 'token') {
    ...
    if (clientAssertion) {
      if (typeof clientAssertion !== 'string' || !clientAssertion.trim()) {
        throw new ValidationError(
          'clientAssertion must be a non-empty string if provided',
          'clientAssertion',
        );
      }
    ...
    try {
      ...
      throw new SDKError(
        'Failed to stringify formData: Unknown error',
